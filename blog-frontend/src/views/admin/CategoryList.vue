<template>
  <div>
    ...
    <div class="add-bar">
      <input v-model="newName" placeholder="输入分类名称后按 Enter 添加" @keyup.enter="handleAdd" />
      ...
      <table>
        ...
        <tbody>
          ...
          <tr v-if="categories.length === 0">
            <td colspan="3" class="empty-row">暂无分类</td>
          </tr>
        </tbody>
      </table>
    </div>
...
async function handleAdd() {
  ...
  await api.post('/admin/categories', { name: newName.value.trim() })
  newName.value = ''
  loadCategories()
}
...
async function handleUpdate(id) {
  await api.put(`/admin/categories/${id}`, { name: editName.value.trim() })
  editId.value = null
  loadCategories()
}
